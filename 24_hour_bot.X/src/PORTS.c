...
/*
//PORT X
static unsigned int * const PORTX_TRISCLR[] = {&TRISFCLR, &TRISBCLR, &TRISGCLR,
                                               &TRISFCLR, &TRISGCLR, &TRISFCLR,
                                               &TRISGCLR, &TRISDCLR, &TRISDCLR,
                                               &TRISDCLR};
static unsigned int * const PORTX_TRISSET[] = {&TRISFSET, &TRISBSET, &TRISGSET,
                                               &TRISFSET, &TRISGSET, &TRISFSET,
                                               &TRISGSET, &TRISDSET, &TRISDSET,
                                               &TRISDSET};
static unsigned int * const PORTX_LATCLR[] = {&LATFCLR, &LATBCLR, &LATGCLR,
                                              &LATFCLR, &LATGCLR, &LATFCLR,
                                              &LATGCLR, &LATDCLR, &LATDCLR,
                                              &LATDCLR};
static unsigned int * const PORTX_LATSET[] = {&LATFSET, &LATBSET, &LATGSET,
                                              &LATFSET, &LATGSET, &LATFSET,
                                              &LATGSET, &LATDSET, &LATDSET,
                                              &LATDSET};
static unsigned int * const PORTX_LATINV[] = {&LATFINV, &LATBINV, &LATGINV,
                                              &LATFINV, &LATGINV, &LATFINV,
                                              &LATGINV, &LATDINV, &LATDINV,
                                              &LATDINV};

//PORT Y
static unsigned int * const PORTY_TRISCLR[] = {&TRISDCLR, &TRISDCLR, &TRISDCLR,
                                               &TRISDCLR, &TRISECLR, &TRISDCLR,
                                               &TRISECLR, &TRISDCLR, &TRISECLR,
                                               &TRISDCLR};
static unsigned int * const PORTY_TRISSET[] = {&TRISDSET, &TRISDSET, &TRISDSET,
                                               &TRISDSET, &TRISESET, &TRISDSET,
                                               &TRISESET, &TRISDSET, &TRISESET,
                                               &TRISDSET};
static unsigned int * const PORTY_LATCLR[] = {&LATDCLR, &LATDCLR, &LATDCLR,
                                              &LATDCLR, &LATECLR, &LATDCLR,
                                              &LATECLR, &LATDCLR, &LATECLR,
                                              &LATDCLR};
static unsigned int * const PORTY_LATSET[] = {&LATDSET, &LATDSET, &LATDSET,
                                              &LATDSET, &LATESET, &LATDSET,
                                              &LATESET, &LATDSET, &LATESET,
                                              &LATDSET};
static unsigned int * const PORTY_LATINV[] = {&LATDINV, &LATDINV, &LATDINV,
                                              &LATDINV, &LATEINV, &LATDINV,
                                              &LATEINV, &LATDINV, &LATEINV,
                                              &LATDINV};

//PORT Z
static unsigned int * const PORTZ_TRISCLR[] = {&TRISECLR, &TRISFCLR, &TRISECLR,
                                               &TRISDCLR, &TRISECLR, &TRISDCLR,
                                               &TRISECLR, &TRISFCLR, &TRISECLR,
                                               &TRISFCLR};
static unsigned int * const PORTZ_TRISSET[] = {&TRISESET, &TRISFSET, &TRISESET,
                                               &TRISDSET, &TRISESET, &TRISDSET,
                                               &TRISESET, &TRISFSET, &TRISESET,
                                               &TRISFSET};
static unsigned int * const PORTZ_LATCLR[] = {&LATECLR, &LATFCLR, &LATECLR,
                                               &LATDCLR, &LATECLR, &LATDCLR,
                                               &LATECLR, &LATFCLR, &LATECLR,
                                               &LATFCLR};
static unsigned int * const PORTZ_LATSET[] = {&LATESET, &LATFSET, &LATESET,
                                               &LATDSET, &LATESET, &LATDSET,
                                               &LATESET, &LATFSET, &LATESET,
                                               &LATFSET};
static unsigned int * const PORTZ_LATINV[] = {&LATEINV, &LATFINV, &LATEINV,
                                               &LATDINV, &LATEINV, &LATDINV,
                                               &LATEINV, &LATFINV, &LATEINV,
                                               &LATFINV};

//PORT V
static unsigned int * const PORTV_TRISCLR[] = {&TRISBCLR, &TRISBCLR, &TRISBCLR,
                                               &TRISBCLR, &TRISBCLR, &TRISBCLR};
static unsigned int * const PORTV_TRISSET[] = {&TRISBSET, &TRISBSET, &TRISBSET,
                                               &TRISBSET, &TRISBSET, &TRISBSET};
static unsigned int * const PORTV_LATCLR[] = {&LATBCLR, &LATBCLR, &LATBCLR,
                                               &LATBCLR, &LATBCLR, &LATBCLR};
static unsigned int * const PORTV_LATSET[] = {&LATBSET, &LATBSET, &LATBSET,
                                               &LATBSET, &LATBSET, &LATBSET};
static unsigned int * const PORTV_LATINV[] = {&LATBINV, &LATBINV, &LATBINV,
                                               &LATBINV, &LATBINV, &LATBINV};

//PORT W
static unsigned int * const PORTW_TRISCLR[] = {&TRISBCLR, &TRISBCLR, &TRISBCLR,
                                               &TRISBCLR, &TRISBCLR, &TRISBCLR};
static unsigned int * const PORTW_TRISSET[] = {&TRISBSET, &TRISBSET, &TRISBSET,
                                               &TRISBSET, &TRISBSET, &TRISBSET};
static unsigned int * const PORTW_LATCLR[] = {&LATBCLR, &LATBCLR, &LATBCLR,
                                               &LATBCLR, &LATBCLR, &LATBCLR};
static unsigned int * const PORTW_LATSET[] = {&LATBSET, &LATBSET, &LATBSET,
                                               &LATBSET, &LATBSET, &LATBSET};
static unsigned int * const PORTW_LATINV[] = {&LATBINV, &LATBINV, &LATBINV,
                                               &LATBINV, &LATBINV, &LATBINV};

static const unsigned short PortVBits[] = {BIT_2,BIT_3,BIT_4,BIT_5,BIT_8,BIT_9};
static const unsigned short PortWBits[] = {BIT_11,BIT_10,BIT_13,BIT_12,BIT_15,BIT_14};
#ifdef JP_SPI_MASTER
static const unsigned short PortXBits[] = {BIT_5,BIT_0,BIT_6,BIT_4,BIT_7,BIT_6,BIT_8,BIT_7,BIT_4,BIT_6};
#else
static const unsigned short PortXBits[] = {BIT_5,BIT_0,BIT_6,BIT_4,BIT_8,BIT_6,BIT_7,BIT_7,BIT_4,BIT_6};
#endif
static const unsigned short PortYBits[] = {BIT_11,BIT_3,BIT_5,BIT_10,BIT_7,BIT_9,BIT_6,BIT_2,BIT_5,BIT_1};
static const unsigned short PortZBits[] = {BIT_4,BIT_1,BIT_3,BIT_0,BIT_2,BIT_8,BIT_1,BIT_3,BIT_0,BIT_2};
*/
